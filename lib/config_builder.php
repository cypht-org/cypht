...
/**
 * Site configuration build steps, shared by scripts/config_gen.php (CLI) and
 * Hm_Installer (in-process). Throws instead of die()/exit() so a failure here
 * never kills the process it's called from.
 */
...
function build_config() {
    ...
    if ((float) substr(phpversion(), 0, 3) < $minVersion) {
        throw new RuntimeException("Cypht requires PHP version $minVersion or greater");
    }
 ...
 *   fatal      — true: abort after reporting; false: warn and continue
...
function check_dependencies($settings) {
    ...
    if (!empty($fatal_missing)) {
        ...
        printf("%s\n", str_repeat('-', 72));
        throw new RuntimeException('Aborting: install the missing required extension(s) and re-run.');
    }
...
function read_mstnef_viewer_config($settings) {
    if ($settings['enable_mstnef_viewer']) {
        if (! check_executable_dependency('tnef')) {
            throw new RuntimeException("'tnef' executable not found. Please install `tnef` cli tool or disable 'enable_mstnef_viewer' to continue.");
        }
        if (! check_executable_dependency('unrtf')) {
            throw new RuntimeException("'unrtf' executable not found. Please install `unrtf` cli tool or disable 'enable_mstnef_viewer' to continue.");
        }
    }
}
