...
trait InteractsWithQueue
{
     ...
     * @return void
     */
    public function push(Queueable $item, $data = '', $queue = null): void
    {
        $driver = $this->getDriver();

        // Call the appropriate method from the QueueManager to push the job
        (new QueueManager)->getDriver($driver)->push($this, $data, $queue);
     ...
     */
    public function pop($queue = null): ?Queueable
    {
        $driver = $this->getDriver();

        // Call the appropriate method from the QueueManager to pop a job
        $item = (new QueueManager)->getDriver($driver)->pop($queue);
    ...
    public function release($queue = null, $delay = 0): void
    {
        ...
        (new QueueManager)->getDriver($driver)->release($this, $queue, $delay);
