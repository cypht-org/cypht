CREATE TABLE hm_user (username VARCHAR(255), hash VARCHAR(255), PRIMARY KEY (username));
CREATE TABLE hm_user_session (hm_id VARCHAR(255), data LONGBLOB, date TIMESTAMP, PRIMARY KEY (hm_id));
CREATE TABLE hm_user_settings (username VARCHAR(255), settings LONGBLOB, PRIMARY KEY (username));
