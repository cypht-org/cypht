...
/**
 * Get message headers for the combined draft page
 * @subpackage imap/handler
 */
class Hm_Handler_imap_draft extends Hm_Handler_Module {
    /**
     * Returns list of message data for the Everthing page
     */
    public function process() {
        list($success, $form) = $this->process_form(array('imap_server_ids'));
        if ($success) {
            $limit = $this->user_config->get('draft_per_source_setting', DEFAULT_PER_SOURCE);
            $date = process_since_argument($this->user_config->get('draft_since_setting', DEFAULT_SINCE));
            $ids = explode(',', $form['imap_server_ids']);
            $folder = bin2hex('INBOX');
            if (array_key_exists('folder', $this->request->post)) {
                $folder = $this->request->post['folder'];
            ...
            $this->out('folder_status', $status);
            $this->out('imap_draft_data', $msg_list);
            $this->out('imap_server_ids', $form['imap_server_ids']);
        }
    }
}
...
class Hm_Handler_load_imap_servers_for_message_list extends Hm_Handler_Module {
    ...
    public function process() {
        ...
        if ($callback) {
            ...
            if ($path == 'sent' || $path == 'drafts') {
                $folder = $path == 'drafts'? 'draft': 'sent';
