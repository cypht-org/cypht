...
var imap_move_copy = function(e, action, context) {
    ...
    if (!e.target || e.target.classList.contains('imap_move')) {
        ...
        move_to = $(e.target).nextAll('.move_to_location').first();
    }
...
var expand_imap_move_to_folders = function(path, context, move_to) {
    var detail = Hm_Utils.parse_folder_path(path, 'imap');
    var folderPart = Hm_Utils.clean_selector(detail.folder);
    var selector = '.imap_' + detail.server_id + '_';
    if (folderPart) {
        selector += folderPart;
    }
    var list = $(selector, move_to);
