    ...
    class Hm_IMAP extends Hm_IMAP_Cache {
        ...
        */
                    ...
                    foreach ($classes as $i => $v) {
                        ...
                        foreach (explode(',', $list) as $val) {
                            ...
                            if (mb_strlen($val) == 1) {
                                ...
                                $delim = mb_substr($val, -1);
                                $prefix = trim(mb_substr($val, 0, -1));
                            }
        ...
        public function get_message_list($uids, $raw=false, $include_content_body = false) {
            ...
            $command .= "BODY.PEEK[HEADER.FIELDS (SUBJECT X-AUTO-BCC FROM DATE CONTENT-TYPE X-PRIORITY TO LIST-ARCHIVE REFERENCES MESSAGE-ID X-SNOOZED X-SCHEDULE X-PROFILE-ID X-DELIVERY X-MS-EXCHANGE-CALENDAR-SERIES-INSTANCE-ID SENDER)]";
            ...
            $junk = array('X-AUTO-BCC', 'MESSAGE-ID', 'REFERENCES', 'X-SNOOZED', 'X-SCHEDULE', 'X-PROFILE-ID', 'X-DELIVERY', 'LIST-ARCHIVE', 'SUBJECT', 'FROM', 'CONTENT-TYPE', 'TO', '(', ')', ']', 'X-PRIORITY', 'DATE', 'X-MS-EXCHANGE-CALENDAR-SERIES-INSTANCE-ID', 'SENDER');
                ...
                if (isset($vals[0]) && $vals[0] == '*') {
                    ...
                    for ($i=0;$i<$count;$i++) {
                        ...
                        elseif ($vals[$i] == 'BODY[1.MIME' || $vals[$i] == 'BODY[2.MIME') {
                            ...
                            $i++;
                            $i++;
                            if (strpos($vals[$i], 'text/calendar') !== false || strpos($vals[$i], '.ics') !== false) {
                        ...
                        elseif ($v != ']' && !$search) {
                            ...
                            if (is_array($parts) && !empty($parts)) {
                                ...
                                foreach ($parts as $line) {
                                    ...
                                    elseif ($split) {
                                        $i++;
                                        $last = mb_substr($line, 0, $split);
                                        $headers[$i] = array($last, trim(mb_substr($line, ($split + 1))));
                                    }
                                }
                            }
                            break;
