...
class Hm_Output_inline_message_setting extends Hm_Output_Module {
    protected function output() {
        ...
        $res = '<tr class="general_setting"><td><div class="d-flex flex-column flex-md-row"><span class="fw-bold me-md-3"><label class="form-check-label" for="inline_message">'.$this->trans('Show messages inline').'</label></span><div><input value="1" type="checkbox" class="form-check-input" name="inline_message" id="inline_message" data-default-value="'.(DEFAULT_INLINE_MESSAGE ? 'true' : 'false') . '"';
        $reset = '';
        if ($inline) {
            $res .= ' checked="checked"';
        }
...
class Hm_Output_inline_message_style extends Hm_Output_Module {
    protected function output() {
        ...
        $res = '<tr class="general_setting"><td><div class="d-flex flex-column flex-md-row"><span class="fw-bold me-md-3">'.$this->trans('Inline Message Style').'</span><div><select class="form-select form-select-sm w-auto" name="inline_message_style" data-default-value="'.DEFAULT_INLINE_MESSAGE_STYLE.'">';
        $res .= '<option ';
        if ($selected == 'right') {
        ...
        $res .= 'value="inline">'.$this->trans('Inline').'</option></select>'.$reset.'</div></div></td></tr>';
        return $res;
    }
}
