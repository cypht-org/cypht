    ...
    protected function output() {
        ...
        if (!empty($msgs)) {
            $res .= implode(',', array_map(function($v) {
                if (preg_match("/ERR/", $v)) {
                    return sprintf('<div class="alert alert-danger alert-dismissible fade show" role="alert"><i class="bi bi-exclamation-triangle me-2"></i><span class="danger" id="sys_messages">%s</span>', $this->trans(substr((string) $v, 3)));
                }
                else {
                    return sprintf('<div class="alert alert-success alert-dismissible fade show" role="alert"><i class="bi bi-check-circle me-2"></i><span class="info" id="sys_messages">%s</span>', $this->trans($v));
                }
            }, $msgs));
    ...
    protected function output() {
        ...
        $res .= '</form>
            ...
            <div class="step_config-actions mt-4 d-flex justify-content-between">
                ...
                <button class="btn btn-primary px-3" id="step_config_action_finish" onclick="display_config_step(3)">'.$this->trans('Finish').'</button>
            </div>
        </div>
        <div id="step_config_0" class="step_config current_config_step">
            <button class="btn btn-primary px-4" id="add_new_server_button" onclick="display_config_step(1)"><i class="bi bi-plus-square-fill me-2"></i> '.$this->trans('Add a new server').'</button>
        </div>
    </div>
</div>
<div class="d-block">';
