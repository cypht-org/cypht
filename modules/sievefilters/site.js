...
function populateFilterFromDraft(filterDraft) {
  ...
  (filterDraft.from || []).forEach((fromVal) => {
    ...
    else if (filterDraft.from_filter_type === "not_matches") op = "!Matches";
  ...
  if (filterDraft.use_subject) {
    (filterDraft.subject_contains || []).forEach((subjectVal) => {
      ...
      $(".add_condition_sieve_filters").last().val("subject").trigger("change");

      let op = "Contains";
      if (filterDraft.subject_filter_type === "not_contains") op = "!Contains";
