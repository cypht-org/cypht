...
class Hm_Handler_process_profile_update extends Hm_Handler_Module {
    public function process() {
        ...
        if (isset($this->request->post['profile_quickly_create']) && $this->request->post['profile_quickly_create'] == "on") {
            $array_profil_imap = explode("|", $this->request->post['profile_imap']);
            if (count($array_profil_imap) == 2) {
                $profile_name = explode("@", $array_profil_imap[1]);
                $this->request->post['profile_name'] = $profile_name[0];
                $this->request->post['profile_address'] = $array_profil_imap[1];
                $this->request->post['profile_replyto'] = $array_profil_imap[1];
            }
        }
...
function profile_form($form_vals, $id, $smtp_servers, $imap_servers, $out_mod) {
    ...
    if (empty($form_vals['id'])) {
        ...
        $res .= '<input class="form-check-input" name="profile_quickly_create" type="checkbox" role="switch" onchange="handleQuicklyCreateProfileCheckboxChange(this)" id="profile_quickly_create">';
